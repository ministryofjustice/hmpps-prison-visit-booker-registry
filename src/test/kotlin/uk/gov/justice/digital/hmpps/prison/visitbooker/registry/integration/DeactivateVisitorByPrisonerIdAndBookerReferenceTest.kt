...
import uk.gov.justice.digital.hmpps.prison.visitbooker.registry.controller.admin.DEACTIVATE_BOOKER_PRISONER_VISITOR_CONTROLLER_PATH
...
class DeactivateVisitorByPrisonerIdAndBookerReferenceTest : IntegrationTestBase() {
  @MockitoSpyBean
  ...
  fun `when prisoner is deactivated for booker all data is persisted and returned correctly`() {
    ...
    verify(telemetryClientSpy, times(1)).trackEvent(
      DEACTIVATED_VISITOR.telemetryEventName,
      mapOf(
    ...
    assertAuditEvent(auditEvents[0], booker.reference, DEACTIVATED_VISITOR, "Visitor ID - ${visitor1.visitorId} deactivated for prisoner - ${prisoner.prisonerId}")
  }
