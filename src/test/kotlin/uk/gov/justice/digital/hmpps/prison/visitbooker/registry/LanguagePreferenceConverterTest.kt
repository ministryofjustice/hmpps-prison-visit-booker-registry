...
import uk.gov.justice.digital.hmpps.prison.visitbooker.registry.enums.LanguagePreferenceConverter
...
class LanguagePreferenceConverterTest {
  ...
  fun `when database value is lowercase en convert to entity attribute returns EN`() {
    ...
    assertThat(result).isEqualTo(LanguagePreference.EN)
  }
  ...
  fun `when database value is lowercase cy convert to entity attribute returns CY`() {
    ...
    assertThat(result).isEqualTo(LanguagePreference.CY)
  }
