...
class ApproveVisitorRequestTest : IntegrationTestBase() {
  ...
  fun `when a visitor request is approved the visitor is linked to the booker and an OK response is returned`() {
    ...
    assertAuditEvent(auditEvents[0], bookerReference, VISITOR_REQUEST_APPROVED_FOR_PRISONER, "Visitor ID - $visitorIdToBeLinked approved (autoApproved = false) for prisoner - $prisonerId, request reference - $requestReference, actionedBy - SYSTEM")
  }
